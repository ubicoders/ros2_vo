...
class CustomDataPublisher : public rclcpp::Node {
public:
  CustomDataPublisher(const std::string &sequence_arg = "")
    ...
    if (left_image_paths_.empty()) {
      throw std::runtime_error(
          "No images were found with the provided parameters.");
    }
...
private:
  void loadImagePaths() {
    // Structure: dataset_root / sequence / left_folder
    auto seq_path = fs::path(dataset_root_) / sequence_;
...
int main(int argc, char **argv) {
  ...
  try {
    auto node = std::make_shared<CustomDataPublisher>(sequence_arg);
    rclcpp::spin(node);
  } catch (const std::exception &ex) {
    RCLCPP_FATAL(rclcpp::get_logger("customdata_publisher"),
                 "Failed to start publisher: %s", ex.what());
    rclcpp::shutdown();
    return EXIT_FAILURE;
  }
  rclcpp::shutdown();
