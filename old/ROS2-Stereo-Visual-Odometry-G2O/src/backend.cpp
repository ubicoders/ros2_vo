...
Backend::Backend(std::shared_ptr<PinholeCamera> camera, std::shared_ptr<Map> map) : camera_(camera), map_(map) {
  ...
  // Choose the solver method (same approach as ba_demo.cpp, adapted for available libraries)
  // "lm_fix6_3" means: Levenberg-Marquardt, fixed-size vertices (6 for pose, 3 for point)
  // Using CSparse solver since it's linked in CMakeLists.txt
  std::string solverName = "lm_fix6_3_csparse";
  g2o::OptimizationAlgorithmProperty solverProperty;
  optimizer_->setAlgorithm(g2o::OptimizationAlgorithmFactory::instance()->construct(solverName, solverProperty));
...
void Backend::updateMap() {
  // Retrieve the current "active" set of keyframes and map points from the Map module.
  // This constitutes the "Sliding Window" for Local Bundle Adjustment.
  // - Active Keyframes: Usually the most recent N frames (e.g., 7) to constrain the latest trajectory.
...
void Backend::optimize(Map::KeyFrameType &activeKeyframes, Map::MapPointType &activeMapPoints) {
  ...
  // 1. Setup Optimizer: We use a SparseOptimizer with a specific solver (Levenberg-Marquardt).
  // 2. Define Vertices (Nodes):
  //    - VertexSE3Expmap: Represents the Camera Pose (6DOF: Rotation + Translation).
  //    - VertexPointXYZ: Represents the 3D Map Point (3DOF: X, Y, Z).
  // 3. Define Edges (Constraints):
  //    - EdgeProjectXYZ2UV: Represents the observation of a 3D point in a 2D image.
  //      It connects a VertexSE3Expmap and a VertexPointXYZ.
  //      The "error" is the reprojection error: difference between projected 3D point and observed 2D pixel.
  // 4. Optimization: The solver adjusts the Vertices to minimize the total error of all Edges.

  std::unordered_map<int32_t, g2o::VertexSE3Expmap *> vertices;
  std::unordered_map<int32_t, g2o::VertexPointXYZ *> mapPointVertices;
  u_int32_t maxKeyFrameId = 0; // Track max ID to avoid collision with map point IDs
  ...
  for (auto &[id, mapPoint] : activeMapPoints) {
    // ID Offset: We add 'maxKeyFrameId + 1' to the point ID to ensure unique IDs in the graph
    // (since G2O requires unique IDs for all vertices, regardless of type).
    ...
    // Since points only connect to frames (and not other points), the Hessian matrix has a sparse structure.
    // Marginalizing points allows us to solve a much smaller system (only poses) first, then back-substitute for
    // points.
    ...
    if (mapPoint->getObserve().size() >= 2) {
      for (auto &observe : mapPoint->getObserve()) {
        ...
        if (mapPointVertices.find(landmarkId) != mapPointVertices.end() &&
          ...
          if (mapPointVertex == nullptr || poseVertexIt == optimizer_->vertices().end()) {
            continue;
          }
          ...
          // Measurement: The actual 2D pixel coordinates (u, v) observed in the image
          measure(0) = feature->point.pt.x;
          measure(1) = feature->point.pt.y;

          edge->setId(edgeSize);
          edge->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(mapPointVertex));
          edge->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(poseVertexIt->second));
          edge->setMeasurement(measure);
          edge->setInformation(Eigen::Matrix2d::Identity());

          // Robust Kernel (Huber): Reduces the influence of outliers (wrong matches) on the optimization
          // If the error is large, the kernel reduces the weight of this edge.
  ...
  std::cout << "Backend: Starting optimization with " << activeKeyframes.size()
            << " active keyframes (IDs: " << frameIds.front() << " to " << frameIds.back() << "), fixing frames "
            << frameIds[0] << " and " << (frameIds.size() >= 2 ? frameIds[1] : 0) << std::endl;
  std::cout << "Backend: " << mapPointVertices.size() << " map points, " << edgeSize << " edges" << std::endl;
  ...
  for (const auto &[id, vertex] : vertices) {
    ...
    activeKeyframes[id]->needUpdate = true; // Flag to notify other modules (e.g., Viewer)
  }
